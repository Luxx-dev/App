...
local function CalculatePosition<X, Y, Z>(x: number, y: number, z: number) : Vector3
...
function EnemyController.GetWaveStatus()
	if Count > 0 then
...
function EnemyController.Init()
	local EnemyCount = 15
	EnemyController.StartWave(EnemyCount)
end

function EnemyController.StartWave(EnemyCount)
...
function EnemyController.SpawnEnemy()
	...
	EnemyController.FindTarget(EnemyIndex)
end

local MoveCd = 0.2
local NextMoveCd = 2
local radius = 20
function EnemyController.FindTarget(EnemyIndex: number)
	local connection
	connection = RunService.Heartbeat:Connect(function(dt)
		EnemyStats.Connections[EnemyIndex] = connection
		
		if EnemyStats.LastMove[EnemyIndex] and os.clock() - EnemyStats.LastMove[EnemyIndex] < MoveCd then return end
		...
		for _, Part in target do
			if Part.Parent:FindFirstChild("Humanoid") and game.Players:GetPlayerFromCharacter(Part.Parent) then
				...
				if (EnemyStats.Targets[EnemyIndex].HumanoidRootPart.Position - model:FindFirstChild("HumanoidRootPart").Position).Magnitude < 1.5 then
					EnemyController.Attack(EnemyIndex)
				end
				...
				EnemyStats.NextMove[EnemyIndex] += dt
...
function EnemyController.Attack(EnemyIndex: number)
...
function EnemyController.Destroy(EnemyIndex: number)
...
function EnemyController.KillAllEnemies()
	for i = Count, 1, -1 do
		EnemyController.Destroy(i)
	end
end
